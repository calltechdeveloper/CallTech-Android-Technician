...
public class MessageFragment extends BaseFragment implements View.OnClickListener{
    ...
    private void setRecyslerViw()
    {
        ...
        getActivity().setTitle(sender_name);
        layoutManager=new LinearLayoutManager(getActivity());
        binding.rvMessage.setLayoutManager(layoutManager);
        messageAdapter =new MessageAdapter(getActivity(),messageList,userPref);
...
// Child Config
    ...
    private void callUploadImage()
    {
       /* showProgressDialog();
        ...
        Call<UploadImageResponse> registrationModelCall= APIExecutor.getApiService(getActivity()).callUploadImage(image,rquest);
    ...
    private void checkPermission()
    {
        ...
        if (!PermissionUtils.checkPermissionCamera(getActivity())) {
            PermissionUtils.requestCameraPermission(getActivity(), PermissionUtils.PERMISSION_REQUEST_CODE);
        }
        else {
            if (!PermissionUtils.checkPermissionStorage(getActivity())) {
                PermissionUtils.requestPermissionStorage(getActivity());
            }
            else
            {
                addPhotoDialog();
    ...
    private void takePicture(String imageName) {
        ...
        if (takePictureIntent.resolveActivity(getContext().getPackageManager()) != null) {
            Uri photoURI = null;
            File photoFile = new File(getActivity().getExternalFilesDir("temp"), imageName + ".jpg");
        ...
        if (requestCode == TakePictureUtils.PICK_GALLERY) {
            if (resultCode == Activity.RESULT_OK) {
                try {
                    ...
                    FileOutputStream fileOutputStream = new FileOutputStream(new File(getActivity().getExternalFilesDir("temp"), imageName + ".jpg"));
                    TakePictureUtils.copyStream(inputStream, fileOutputStream);
                    fileOutputStream.close();
                    inputStream.close();
                    startCropImage((Activity) getActivity(), imageName + ".jpg");
                } catch (Exception e) {
                    Toast.makeText(getActivity(), "Error in picture", Toast.LENGTH_SHORT).show();
    ...
    private void callAwardJobApi(final String ammount)
    {
        ...
        Call<CommonResponse> commonResponseCall= APIExecutor.getApiService(getActivity()).callAwardJob(jsonObject);
    ...
    {
        final AwardJobBinding bindingDialog= DataBindingUtil.inflate(LayoutInflater.from(getActivity()), R.layout.award_job, null, false);
        ...
        bindingDialog.btOk.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                {
                    CommonUtils.showSnack(bindingDialog.getRoot(),getActivity().getString(R.string.enter_amount_that_you_discuss_with_employee_to_pay_for_this_job));
                }
                else
                {
                    dialog.dismiss();
